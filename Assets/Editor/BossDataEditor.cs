...
public class BossDataEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        // Boss Presentation fields (title + name + sprite)
        EditorGUILayout.Space();
        EditorGUILayout.LabelField("Boss Presentation", EditorStyles.boldLabel);
        EditorGUILayout.PropertyField(serializedObject.FindProperty("bossTitle"));
        EditorGUILayout.PropertyField(serializedObject.FindProperty("bossName"));
        EditorGUILayout.PropertyField(serializedObject.FindProperty("bossSprite"));
        ...
        // 맵 크기 제한
        if (bossData.mapWidth < 1) bossData.mapWidth = 1;
        if (bossData.mapHeight < 1) bossData.mapHeight = 1;

        EditorGUILayout.Space();
        EditorGUILayout.LabelField("Boss Piece Placement (Visual Editor)", EditorStyles.boldLabel);
        EditorGUILayout.HelpBox($"Grid Range: X[{bossData.minX} ~ {bossData.minX + bossData.mapWidth - 1}], Y[{bossData.minY} ~ {bossData.minY + bossData.mapHeight - 1}]\nLeft Click: Cycle Piece\nRight Click: Clear Piece", MessageType.Info);
        ...
        // 3. 리스트 데이터 동기화
        EditorGUILayout.Space();
        EditorGUILayout.PropertyField(serializedObject.FindProperty("bossPieces"), true);
    ...
    {
        if (bossData.bossPieces == null)
            bossData.bossPieces = new List<PieceSpawner.PieceSpawnInfo>();

        // 현재 데이터를 딕셔너리로 변환하여 빠른 조회
        Dictionary<Vector2Int, PieceType> pieceMap = new Dictionary<Vector2Int, PieceType>();
        foreach (var info in bossData.bossPieces)
        {
        ...
        // 실제 좌표는 minY + y
        for (int y = bossData.mapHeight - 1; y >= 0; y--)
        {
            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();

            for (int x = 0; x < bossData.mapWidth; x++)
            {
                // 실제 게임 내 좌표 계산 (Offset 적용)
                int realX = bossData.minX + x;
                int realY = bossData.minY + y;
    ...
    private void SyncWithGridManager()
    {
        ...
        if (gridManager != null)
        {
            Undo.RecordObject(bossData, "Sync Grid Settings");
            
            bossData.mapWidth = gridManager.boardWidth;
            bossData.mapHeight = gridManager.boardHeight;
            bossData.minX = gridManager.gridMinBounds.x;
            bossData.minY = gridManager.gridMinBounds.y;
    ...
    {
        Undo.RecordObject(bossData, "Modify Boss Data");

        bossData.bossPieces.RemoveAll(p => p.gridCoordinate == pos);
        ...
        bossData.bossPieces.Add(new PieceSpawner.PieceSpawnInfo
        {
            gridCoordinate = pos,
            pieceType = nextType,
    ...
    {
        Undo.RecordObject(bossData, "Remove Piece");
        bossData.bossPieces.RemoveAll(p => p.gridCoordinate == pos);
    }
